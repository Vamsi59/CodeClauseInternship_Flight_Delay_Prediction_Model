...
def loadData():
    flights = pd.read_csv('flights.csv',low_memory=False,nrows=1000)
...
def main():
    ...
    st.subheader("Prediction using own Machine Learning Algorithm")
    gif_url = "https://cdnb.artstation.com/p/assets/images/images/010/497/015/original/tzu-yu-kao-at2-airplane-in-the-sky.gif?1524733234"  # Replace with the actual GIF URL

    st.image(gif_url, use_column_width=False, width=350)
    choice= st.selectbox("Choose Machine Learning Model",["None","Random Forest Regressor","Support Vector Regressor","Gradient Boosting Regressor"])
    final_data = loadData()
    ...
    elif choice == "Support Vector Regressor":
        reg_svr = svr(X_train, y_train)

        month, day, sch_dept, distance, arrival_delay, airline, origin, destination, day_of_week = accept_data()

        if st.button("Predict using Support Vector Regressor"):
            res = prediction(X, month, day, sch_dept, distance, arrival_delay, airline, origin, destination, day_of_week, reg_svr)
            if res >= 0:
                text1 = "Flight is not delayed. It will depart for next flight at scheduled time"
                st.write(text1)
            elif res >= -15:
                text2 = "Flight is only delayed by " + str(abs(res)) + ". Delays up to 15 minutes are considered as not delay. FLIGHT IS NOT DELAYED"
                st.write(text2)
            else:
                text3 = "Flight is delayed by " + str(res) + ". Delays by more than 15 minutes are considered to be actual delays. FLIGHT IS DELAY"
                st.write(text3)
    elif choice == "Gradient Boosting Regressor":
        reg_gb = gradient_boosting(X_train, y_train)

        month, day, sch_dept, distance, arrival_delay, airline, origin, destination, day_of_week = accept_data()

        if st.button("Predict using Gradient Boosting Regressor"):
            res = prediction(X, month, day, sch_dept, distance, arrival_delay, airline, origin, destination, day_of_week, reg_gb)
            if res >= 0:
                text1 = "Flight is not delayed. It will depart for the next flight at the scheduled time."
                st.write(text1)
            elif res >= -15:
                text2 = "Flight is only delayed by " + str(abs(res)) + " minutes. Delays up to 15 minutes are considered as not delayed. FLIGHT IS NOT DELAYED."
                st.write(text2)
            else:
                text3 = "Flight is delayed by " + str(res) + " minutes. Delays by more than 15 minutes are considered to be actual delays. FLIGHT IS DELAYED."
                st.write(text3)
